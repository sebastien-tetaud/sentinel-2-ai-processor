...
def save_config_to_log(config, log_dir, filename="config.yaml"):
    """
    Saves the configuration dictionary to a YAML file in the specified log directory.

    Args:
        config (dict): Configuration dictionary.
        log_dir (str): Path to the log directory (e.g., `result_dir` or `metrics_path`).
        filename (str): Name of the config file to save. Default is "config.yaml".
    """
...
def prepare_data(config):
    ...
    df_train = pd.read_csv(f"{base_dir}/{version}/train_path.csv")
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        for band in bands:
            logger.info(
                f"  {band}: PSNR={val_metrics[band]['psnr']:.2f}, "
                f"RMSE={val_metrics[band]['rmse']:.4f}, "
                f"SSIM={val_metrics[band]['ssim']:.4f}, "
                f"SAM={val_metrics[band]['sam']:.2f}")
